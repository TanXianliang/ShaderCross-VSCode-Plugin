...
`;
	...
	getWebviewContent(webviewView) {
		// 获取webview.html文件的URI
		const htmlPath = vscode.Uri.file(
			path.join(this.context.extensionPath, 'webview.html')
		);
		// 将本地文件路径转换为webview可以访问的URI
		const htmlUri = webviewView.webview.asWebviewUri(htmlPath);
		// 返回HTML内容，通过iframe加载webview.html
		return `
		<!DOCTYPE html>
		<html lang="en">
		<head>
			...
			<title>Shader Cross</title>
		</head>
		<body style="margin: 0; padding: 0;">
			<iframe src="${htmlUri}" style="width: 100%; height: 100vh; border: none;"></iframe>
		</body>
		</html>
		`;
	}
